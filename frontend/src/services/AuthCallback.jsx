...
import { Paper, Typography, CircularProgress, Box, Button, Grid } from '@mui/material';
...
const roleData = Object.keys(roleConfig).map(key => ({ key, ...roleConfig[key] }));
...
const titleBoxStyles = {
    pb: 4, textAlign: 'center'
};
const cardImageContainerStyles = {
    width: '100%', height: 220, background: '#fff',
    display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden'
};
const cardContentStyles = {
    flex: 1, width: '100%', display: 'flex', flexDirection: 'column',
    alignItems: 'center', justifyContent: 'space-around', p: 3, textAlign: 'center'
};
const cardButtonStyles = {
    width: '80%', fontWeight: 700, fontSize: 18, py: 1.5, borderRadius: 2
};
// --- COMPONENTS ---
...
const RoleCard = React.memo(({ role, onSelect, isLoading, isThisCardLoading }) => {
    const [isHovered, setHovered] = useState(false);

    const paperStyles = useMemo(() => ({
        cursor: isLoading ? 'not-allowed' : 'pointer',
        p: 0, minHeight: 520, width: '100%', maxWidth: 400,
        display: 'flex', flexDirection: 'column',
        background: role.bg, borderRadius: 5, overflow: 'hidden',
        ...
        <Grid item xs={12} sm={8} md={4} sx={{ display: 'flex', justifyContent: 'center' }}>
            ...
            >
                <Box sx={cardImageContainerStyles}>
                    <img src={role.img} alt={role.label} style={{ width: 'auto', height: '100%', objectFit: 'cover' }} />
                </Box>
                <Box sx={cardContentStyles}>
                    <Typography variant="h5" fontWeight={700} sx={{ mb: 1, color: '#1e3c72' }}>
                        {role.label}
                    </Typography>
                    <Typography sx={{ color: '#444', mb: 3, fontSize: 16, minHeight: 48 }}>
                        {role.desc}
                    </Typography>
                    <Button
                        ...
                        sx={cardButtonStyles}
                    >
                        {isThisCardLoading ? <CircularProgress size={24} color="inherit" /> : 'Chọn'}
                    </Button>
                </Box>
            </Paper>
        </Grid>
...
const RoleSelectionScreen = React.memo(({ onSelectRole, isLoading, error, message, selectedRole }) => {
    return (
        <Box sx={selectionScreenStyles}>
            <Box sx={titleBoxStyles}>
                <Typography variant="h2" fontWeight={900} color="#1e3c72" sx={{ mb: 1 }}>
                    Julie Academy
                </Typography>
            ...
            {error && <Typography align="center" sx={{ color: 'red', mb: 2 }}>{error}</Typography>}
            {message && <Typography align="center" sx={{ color: 'green', mb: 2 }}>{message}</Typography>}

            <Grid container spacing={4} justifyContent="center" alignItems="stretch" sx={{ maxWidth: 1400 }}>
                {roleData.map((role) => (
                    <RoleCard
                        key={role.key}
                        role={role}
...
export default function AuthCallback() {
    ...
    useEffect(() => {
        ...
        const assignedRole = Object.keys(roleConfig).find(role => userRoles.includes(role));
    ...
    const handleRoleSelection = useCallback(async (roleName) => {
        setLoading(true);
